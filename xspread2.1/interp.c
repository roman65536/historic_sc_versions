...
static double	doprod(int, int, int, int);
static double	domax(int, int, int, int);
static double	domin(int, int, int, int);
static double	donval(char *, double);
static double	dostddev(int, int, int, int);
static char *	dostindex(double, int, int, int, int);
static char *	dosubstr(char *, int, int);
static double	dosum(int, int, int, int);
...
static void	RealEvalOne();
static int	constant();
static void	copyrtv();
static void	decodev();
static void	decompile();
static void	decompile_list();
static double	doavg();
static char *	docapital();
static char *	docase();
static char *	docat();
static double	docount();
static char *	dodate();
static double	doeqs();
static char *	doext();
static char *	dofmt();
static double	doindex();
static double	dolookup();
static double	dolmax();
static double	dolmin();
static double	domax();
static double	domin();
static double	donval();
static double	doprod();
static double	dostddev();
static char *	dostindex();
static char *	dosubstr();
static double	dosum();
static char *	dosval();
static double	doston();
static double	dotime();
static double	dotts();
static double	eval();
static sig_type	eval_fpe();	/* Trap for FPE errors in eval */
static double	finfunc();
static double	fn1_eval();
static double	fn2_eval();
static struct ent *getent();
static void	g_free();
static void	g_last();
static void	index_arg();
static void	list_arg();
static void	one_arg();
static void	range_arg();
static char *	seval();
#ifdef RINT   /*  Otherwise we use the one in math.h */
       double	rint();
#endif
static void	three_arg();
static void	two_arg();
static void	two_arg_index();
		...
		if (minc>maxc) c = maxc, maxc = minc, minc = c;
	        switch (e->op) {
	            case REDUCE | '+': return dosum(minr, minc, maxr, maxc);
 	            ...
 	            case REDUCE | MAXR: return domax(minr, minc, maxr, maxc);
 	            case REDUCE | MINR: return domin(minr, minc, maxr, maxc);
		}
	    }
...
#ifndef DOINGX                     /*ifndef added by Bob Parbs 12-92 */
	    (void) refresh();
#endif

...
bool
format(char *fmt, double val, char *buf, int buflen)
{
    register char *cp;
    char *tmp, *tp;
    ...
    int width;
    char prtfmt[32];
    static char *mantissa;
    static char *tmpfmt1, *tmpfmt2, *exptmp;
    static unsigned mantlen, fmtlen;
    char *fraction = NULL;
    int zero_pad = 0;
...
bool
engformat (int fmt, int width, int lprecision, double val, 
	   char *buf, int buflen)
{
	...
	case REFMTEXP:
		...
		case 1:
		    ...
		    *++pdecimal = 056;	/* write in a new decimal point */
		     ...
		     */
		    if (*(pexponent + 3) == '\0')
			(void) sprintf (pexponent, "%0+3d", exponent);
		    else
			(void) sprintf (pexponent, "%0+4d", exponent);
		    break;
		case 0:
		default:
		    /*

/* xspread data preparation program
 *
 * usage xsdata options
 * options:
 ...
 *  Further changes: Allin Cottrell
 */
...
main(int argc, char **argv)
{
	...
	default:
	    (void) fprintf(stderr, "Usage: %s [-rkfLSPv] [-s v] [-R i] "
			   "[-C i] [-n i] [-d c]\n", progname);
	    exit(EXIT_FAILURE);
        }
    }
    ...
    if (optind < argc) {
	(void) fprintf(stderr, "Usage: %s [-rL] [-s v] [-R i] [-C i] "
		       "[-n i] [-d c]\n", progname);
	exit(EXIT_FAILURE);
    }
	...
	switch (scan()) {
	case END:
	    if(drop_format) exit(EXIT_SUCCESS);
	    for (i = 0; i<maxcols; i++) {
		if (fwidth[i])

//
// "$Id: fl_ag_file_chooser.cxx,v 1.3 2001/05/29 15:57:59 cottrell Exp $"
//
...
class FCB : public Fl_Ag_Browser_ {
...
public:
  ...
  int prev_count;
  FCB(int x, int y, int w, int h) : Fl_Ag_Browser_(x, y, w, h, 0) {
 ...
 BREAK:
  ...
  if (bufdirend != dirend || strncmp(buf, listed, bufdirend)) {
	...
	prev_last = last;
      ...
      int n = fl_filename_list(dirend ? listed : ".", &list);
      if (n < 0) {
...
#define HEIGHT_BUT	17
#define HEIGHT_INPUT	17
#define WIDTH_BOX	(3*WIDTH_SPC+WIDTH_BUT+WIDTH_BROWSER)
#define WIDTH_BUT	48
#define WIDTH_OK	48
#define WIDTH_SPC	2
#ifdef VR3 // Agenda vr3: tiny 
# define HEIGHT_BROWSER	(7*HEIGHT_BUT+2) // must be > buttons*HEIGHT_BUT
# define WIDTH_BROWSER	106
#else // "regular" size
# define HEIGHT_BROWSER	(14*HEIGHT_BUT+2) // must be > buttons*HEIGHT_BUT
# define WIDTH_BROWSER	240
#endif // not VR3
		...
		WIDTH_BROWSER, HEIGHT_BROWSER)
{
  ...
  obj=new Fl_Button(WIDTH_SPC,but_y,WIDTH_BUT,HEIGHT_BUT, "&Up ..");
  ...
  obj=new Fl_Button(WIDTH_SPC, but_y, WIDTH_BUT, HEIGHT_BUT, "&CWD");
  ...
  normal_button->labelsize(10);
  but_y += HEIGHT_BUT;

  obj = new Fl_Button(WIDTH_SPC,but_y, WIDTH_BUT, HEIGHT_BUT, "* &All");
  obj->callback(files_cb, this);
  obj->labelsize(10);
  but_y += HEIGHT_BUT;

  obj = new Fl_Button(WIDTH_SPC,but_y,WIDTH_BUT,HEIGHT_BUT, ". &Hidden");
  obj->callback(files_cb, this);
  obj->labelsize(10);
  but_y += HEIGHT_BUT;

  obj = new Fl_Button(WIDTH_SPC,but_y,WIDTH_BUT,HEIGHT_BUT, "*/ &Dirs");
  obj->callback(files_cb, this);
  obj->labelsize(10);
  but_y += HEIGHT_BUT;
...
char* fl_ag_file_chooser(const char* message, const char* pat, 
			 const char* fname)
{
  static FCW* f; if (!f) f = new FCW();
  f->ok_button->label("OK  ");
  f->cancel_button->label(fl_cancel);

  // *winid = fl_xid(FlWindow);
  ...
  f->normal_button->label(pat ? pat : "Normal");
...
//
// End of "$Id: fl_ag_file_chooser.cxx,v 1.3 2001/05/29 15:57:59 cottrell Exp $".
//

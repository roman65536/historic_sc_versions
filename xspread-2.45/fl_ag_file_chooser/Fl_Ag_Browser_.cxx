//
// "$Id: Fl_Ag_Browser_.cxx,v 1.2 2001/06/01 01:56:28 cottrell Exp $"
//
...
static void scrollbar_callback(Fl_Widget* s, void*) {
  ((Fl_Ag_Browser_*)(s->parent()))->position(int(((Fl_Scrollbar*)s)->value()));
}

static void hscrollbar_callback(Fl_Widget* s, void*) {
  ((Fl_Ag_Browser_*)(s->parent()))->hposition(int(((Fl_Scrollbar*)s)->value()));
}

#ifdef VR3
int Fl_Ag_Browser_::scrollbar_width_ = 10;
#else
int Fl_Ag_Browser_::scrollbar_width_ = 16;
#endif

// return where to draw the actual box:
void Fl_Ag_Browser_::bbox(int& X, int& Y, int& W, int& H) const {
...
int Fl_Ag_Browser_::leftedge() const {
...
void Fl_Ag_Browser_::resize(int X, int Y, int W, int H) {
...
void Fl_Ag_Browser_::redraw_line(void* l) {
...
void Fl_Ag_Browser_::update_top() {
...
void Fl_Ag_Browser_::position(int y) {
  if (y < 0) y = 0;
...
void Fl_Ag_Browser_::hposition(int x) {
  if (x < 0) x = 0;
...
int Fl_Ag_Browser_::displayed(void* x) const {
  int X, Y, W, H; bbox(X, Y, W, H);
  int yy = H+offset_;
...
void Fl_Ag_Browser_::display(void* x) {
  update_top();
...
// all pointers to it:
void Fl_Ag_Browser_::deleting(void* l) {
...
void Fl_Ag_Browser_::replacing(void* a, void* b) {
  redraw_line(a);
...
void Fl_Ag_Browser_::inserting(void* a, void* b) {
  if (displayed(a)) redraw_lines();
  if (a == top_) top_ = b;
}

void* Fl_Ag_Browser_::find_item(int my) {
  update_top();
  int X, Y, W, H; bbox(X, Y, W, H);
  void* l;
  int yy = Y-offset_;
...
int Fl_Ag_Browser_::select(void* l, int i, int docallbacks) {
...
int Fl_Ag_Browser_::deselect(int docallbacks) {
  if (type() == FL_MULTI_BROWSER) {
    int change = 0;
...
int Fl_Ag_Browser_::select_only(void* l, int docallbacks) {
...
int Fl_Ag_Browser_::handle(int event) {
...
Fl_Ag_Browser_::Fl_Ag_Browser_(int x, int y, int w, int h, const char* l)
  : Fl_Group(x, y, w, h, l),
...
int Fl_Ag_Browser_::item_quick_height(void* l) const {
  return item_height(l);
}

int Fl_Ag_Browser_::incr_height() const {
  return item_quick_height(item_first());
}

int Fl_Ag_Browser_::full_height() const {
  int t = 0;
  for (void* p = item_first(); p; p = item_next(p))
...
int Fl_Ag_Browser_::full_width() const {
  return max_width;
}

void Fl_Ag_Browser_::item_select(void*, int) {}

int Fl_Ag_Browser_::item_selected(void* l) const {return l==selection_;}

//
// End of "$Id: Fl_Ag_Browser_.cxx,v 1.2 2001/06/01 01:56:28 cottrell Exp $".
//
